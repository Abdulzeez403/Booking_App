...
import PropertyCard from "../compnonents/PropertyCard";
import {
  BottomModal,
  ModalContent,
  ModalFooter,
...
const PlacesScreen = () => {
  ...
  const ApplyFilter = (filter) => {
    ...
    switch (filter) {
      ...
      case "Low to High":
        searchPlaces.map((val) => val.properties.sort(comparion));
        setSortedData(searchPlaces);
        break;
    }
  };
    ...
    <View>
      <Pressable style={styles.MenuContainer}>
        ...
        >
          ...
          <FontAwesome name="sort-amount-desc" size={22} color="gray" />
        </Pressable>
        <Pressable style={styles.menuStyle}>
          <Text style={styles.menuText}>Filter</Text>
          <Ionicons name="ios-filter" size={22} color="gray" />
        </Pressable>
        <Pressable
          onPress={() =>
      ...
      <BottomModal
        ...
        modalTitle={<ModalTitle title="Sort and Filter By Price" />}
        modalAnimation={
          new SlideAnimation({
            slideFrom: "bottom",
          })
